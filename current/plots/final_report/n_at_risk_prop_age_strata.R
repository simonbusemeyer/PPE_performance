# n_at_risk_prop_age_strata.R

plot_n_at_risk_prop_by_age <- function(
    results_df,
    title = "Patients at Risk (Proportion of Initial Cohort) by Age Class",
    x_lab = "Proportion of Cohort Experiencing Cancer Death vs Non-Cancer Death",
    y_lab = "Proportion at Risk (Mean n_at_risk / Mean n_patients)",
  ...
  results_plot_df <- results_df |> 
    ...
    dplyr::mutate(
      prop_mean_n_at_risk = mean_n_at_risk / mean_n_patients,
  ...
  ggplot2::ggplot(
    results_plot_df,
    ggplot2::aes(x = pct_cancer, y = prop_mean_n_at_risk)
  ) +
    ggplot2::geom_point(
      ggplot2::aes(
        color = time_t_factor,

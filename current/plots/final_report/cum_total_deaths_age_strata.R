# cum_total_deaths_age_strata.R

plot_cum_total_deaths_by_age <- function(
    results_df,
    title = "Cumulative Total Deaths by Age Class",
    x_lab = "Proportion of Cohort Experiencing Cancer Death vs Non-Cancer Death",
    y_lab = "Mean Cumulative Total Deaths",
  ...
  results_plot_df <- results_df |> 
    ...
    dplyr::mutate(
      mean_cum_total_deaths = mean_cum_total_deaths,
  ...
  ggplot2::ggplot(
    results_plot_df,
    ggplot2::aes(x = pct_cancer, y = mean_cum_total_deaths)
  ) +
    ggplot2::geom_point(
      ggplot2::aes(
        color = time_t_factor,

# cum_cancer_prop_age_strata.R

plot_cum_cancer_prop_by_age <- function(
    results_df,
    title = "Cumulative Cancer Deaths (Proportion of Initial Cohort) by Age Class",
    x_lab = "Proportion of Cohort Experiencing Cancer Death vs Non-Cancer Death",
    y_lab = "Proportion of Initial Cohort (Mean Cum. Cancer Deaths)",
  ...
  results_plot_df <- results_df |> 
    ...
    dplyr::mutate(
      prop_cum_cancer = mean_cum_cancer / mean_n_patients,
  ...
  ggplot2::ggplot(
    results_plot_df,
    ggplot2::aes(x = pct_cancer, y = prop_cum_cancer)
  ) +
    ggplot2::geom_point(
      ggplot2::aes(
        color = time_t_factor,

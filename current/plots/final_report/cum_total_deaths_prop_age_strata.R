# cum_total_deaths_prop_age_strata.R

plot_cum_total_deaths_prop_by_age <- function(
    results_df,
    title = "Cumulative Total Deaths (Proportion of Initial Cohort) by Age Class",
    x_lab = "Proportion of Cohort Experiencing Cancer Death vs Non-Cancer Death",
    y_lab = "Proportion of Initial Cohort (Mean Cum. Total Deaths)",
    followup_lab = "Follow-up"
) {
  
  required_cols <- c("time_t", "pct_cancer", "mean_cum_total_deaths", "mean_n_patients", "pp_age_class")
  ...
  results_plot_df <- results_df |> 
    ...
    dplyr::mutate(
      prop_mean_cum_total_deaths = mean_cum_total_deaths / mean_n_patients,
  ...
  ggplot2::ggplot(
    results_plot_df,
    ggplot2::aes(x = pct_cancer, y = prop_mean_cum_total_deaths)
  ) +
    ggplot2::geom_point(
      ggplot2::aes(
        color = time_t_factor,
